...
@virtualize
class InterpCcTaintTest extends TutorialFunSuite {
  val under = "interpcc_taint_"
...
int main(int argc, char* argv[]) {
  ...
  for (i=0; i<SECRET_SIZE; i++) {
    s1.mem[i+SECRET_OFFSET] = source();
  }
  struct stateT s1_ = Snippet(s1);

  return sink(s1_.timer);
}
"""
  ...
  test("interp ni time tainted") {
    ...
    check("should_leak", snippet.code)
  }
}
